...
CV_table = read.table("/Users/yangwen/Desktop/Research/Programing/Data/Critical value with 2n.csv",
                      header = TRUE,
                      sep = ",")
# N = 70 # N is the number of training sample
...
for (m in 1:M){
  ...
  N = 200 # N is the length of the training sample
  ...
  for (i in 1:B){
    ...
    tTstat = rep(0,(N-2*n+1))
    tTstat2 = rep(0,(N-2*n+1))
    tTstatA = rep(0,(N-2*n+1))
    tTstatA2 = rep(0,(N-2*n+1))
    
    for (j in 1:(N-2*n+1))
    {
      
      if (d>1) {
        Bbivn = BbivnRds[j:(j+2*n-1),]
        Bbivn1 = BbivnRds[j:(j+n-1),]
        Bbivn2 = BbivnRds[(j+n):(j+2*n-1),] 
      } else { 
        Bbivn = BbivnRds[j:(j+2*n-1)]
        Bbivn1 = BbivnRds[j:(j+n-1)]
        Bbivn2 = BbivnRds[(j+n):(j+2*n-1)]   
...
write.table(Performance, file = "/Users/yangwen/Desktop/Research/Programing/Data/OnlinePermuCali with HomoVar_B1000.csv", sep = ",", col.names = NA,
            qmethod = "double")
