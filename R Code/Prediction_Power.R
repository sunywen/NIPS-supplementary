...
CV_table = read.table("/Users/yangwen/Desktop/Research/Bootstrap_CV_Homo1.csv",
                      header = TRUE,
                      sep = ",")
...
for (m in 1:M){
  ...
  for (i in 1:N){
    ...
    mu2 = rep(I_true[i]/sqrt(d), d)
    Sigma2 = diag(1,d) #* (I_true[i]*3+1)
...
Performance = data.frame(d=CV_table$d,n=CV_table$n,AccuracyMST,SensitivityMST,AccuracyAC,SensitivityAC,AccuracyR,SensitivityR)
saveRDS(Performance,file="/Users/yangwen/Desktop/Research/Performance_Mean1.Rda")
write.table(Performance, file = "/Users/yangwen/Desktop/Research/Performance_MeanSqrt1_100.csv", sep = ",", col.names = NA,
            qmethod = "double")
